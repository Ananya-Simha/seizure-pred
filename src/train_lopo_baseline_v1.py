...
def main():
    ...
    all_patients = [f"chb{str(i).zfill(2)}" for i in range(1, 11)]  # chb01–chb010
    ...
    for test_patient in all_patients:
        ...
        optimizer = torch.optim.Adam(
            list(model_feat.parameters()) + list(model_clf.parameters()),
            lr=1e-3
        )
    ...
    # Save results to CSV
    import csv
    csv_path = os.path.join(os.path.dirname(__file__), "baseline_lopo_results.csv")
